...
void main()
{
    ...
    if(inputColor > 0.0){
        meVal.rgb = vec3(colorR, colorG, colorB);
    }
...
#ifdef USE_SEG
    if (meVal.a < 0.001) discard;      float seg_opacity = (texture2D(segMaskTexture, segCoord)).x;
    ...
    gl_FragColor = vec4(mix(colorRes, color, alpha), 1.0) * src.a;    
}
